...
function bgFetch(param29, value20 = {}) {
  return new Promise((param30, param31) => {
    ...
    console.log("[content.js][bgFetch] >>> enviando para background", {
      url: msg.url,
      method: msg.method,
      bodyPreview: msg.body ? msg.body.slice(0, 200) : null,
    });
    chrome.runtime.sendMessage(msg, (param32) => {
      if (!param32) {
          console.warn("[bgFetch] resposta vazia (undefined/null)");
          return param31(new Error("No response from background"));
        }
        console.log("[content.js][bgFetch] <<< resposta do background", {
          ok: param32.ok,
          status: param32.status,
          dataKeys: param32.data
            ? Object.keys(param32.data).join(",")
            : null,
        });
        if (param32.data && typeof param32.data === "object") {
          if (!param32.ok) {
            ...
            console.error(
              "[content.js][bgFetch] HTTP " + param32.status + " →",
              param32.data,
            );
            return param31(
...
async function validateLicense() {
  ...
  console.log("[content.js][validateLicense] >>> botão clicado", {
    keyProvided: !!value33,
    keyPreview: value33 ? value33.slice(0, 4) + "****" : null,
    deviceId: qlDeviceId ? qlDeviceId.slice(0, 8) + "****" : "(será obtido)",
  });
  if (!value33) {
    if (value32) {
  ...
  try {
    ...
    console.log("[content.js][validateLicense] chamando EUBackend.validateLicense", {
      deviceId: qlDeviceId.slice(0, 8) + "****",
    });
    const value35 = await window.EUBackend.validateLicense(value33, {
      deviceId: qlDeviceId,
    });
    console.log("[content.js][validateLicense] <<< resultado", value35);
    if (value35.valid) {
