    ...
    if (Object.keys(config1).length) {
      ...
      chrome.storage.local.get(["eu_license_key", "ql_license_key", "eu_device_id", "ql_device_id"], (items) => {
        ...
        fetch("https://lovable-spark-saas.lovable.app/api/public/license/heartbeat", {
          ...
          body: JSON.stringify({
            key: licenseKey,
            device_id: items.eu_device_id || items.ql_device_id || "",
          }),
        }).catch((error) => {
    ...
    (async () => {
      try {
        const items3 = [
          ...
          "api.lovable.dev",
          "lovable-api.com",
          "lovable-spark-saas.lovable.app",
        ];
        let text1 = "";
        try {
        ...
        console.log("[Background] proxyFetch >>>", {
          url: param9.url,
          method: param9.method,
          bodyPreview: param9.body ? param9.body.slice(0, 200) : null,
        });
        var config2 = {
        ...
        console.log("[Background] proxyFetch <<< resposta bruta", {
          status: value1.status,
          bodyPreview: value2.slice(0, 500),
        });
        var value3;
        try {
        ...
        console.log("[Background] proxyFetch <<< resposta final", {
          ok: value1.ok,
          status: value1.status,
          data: value3,
        });
